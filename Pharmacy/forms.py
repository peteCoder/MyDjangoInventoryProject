...
class AddCategoryForm(forms.ModelForm):
	...
	def clean_name(self):
		...
		for instance in PharmaceuticalCategory.objects.all():
...
class AddProductTypeForm(forms.ModelForm):
	...
	def clean_name(self):
		...
		for instance in PharmaceuticalType.objects.all():
			if instance.name == name:
				raise forms.ValidationError(name + " already exists.")
		return name
...
class EditProductForm(forms.ModelForm):
	class Meta:
		...
		widgets = {
			...
			'medicine_require_prescription': forms.CheckboxInput(attrs={
				'type': 'checkbox',
...
class AddProductForm(forms.ModelForm):
	...
	def clean_product_name(self):
		...
		if not product_name or product_name == "":
			...
			for instance in Pharmaceuticals.objects.all():
...
class SalesForm(forms.ModelForm):
	...
	def clean(self):
		...
		drug = ProcurementList.objects.get(purchase_ref=purchase_ref)

		drug_available = drug.quantity_ordered
		...
		left_in_stock = drug_available - total_q_sold

		if drug.product_has_expired_already:
		...
		if int(total_q_sold + quantity_sold) > int(drug_available):
		...
		if int(drug_available) < int(quantity_sold):
			raise forms.ValidationError({'quantity_sold':f"{drug.quantity_ordered} {p_name} \
			 available in stock. \
